...
Loading the helper functions. Used references [6] and [7] in constructing these. 
...
mean_box <- function(ds, dsvcol, dsccol) {
 ...
 fa2 <- ggplot(aes_string(x=dsccol, y=dsvcol), data=ds) +
...
Only a slight skew to the right. 
...
q1 <- quickhist(ww, ww$quality)
q2 <- ggplot(aes(x = quality), data = ww) +
  geom_histogram(binwidth = 0.001, color = 'black', fill = '#099DD9') +
...
Confirms that the ratings are whole integers. 
...
# For convenience, shorten the sulfur dioxide column names
...
ggplot(aes(x = quality.cat, y = density), data = ww) +
  geom_boxplot(color = 'black', fill = '#099DD9') + 
  coord_cartesian(ylim = (xlim = c(0.99, 1.005)));
...
ggplot(aes(x = quality.cat, y = free.so2), data = ww) +
  geom_boxplot(color = 'black', fill = '#099DD9') + 
  coord_cartesian(ylim = (xlim = c(0, 100)));
...
ggplot(aes(x = quality.cat, y = chlorides), data = ww) +
  geom_boxplot(color = 'black', fill = '#099DD9') + 
  coord_cartesian(ylim = (xlim = c(0, 0.1)));
...
ggplot(aes(x = quality.cat, y = sugar), data = ww) +
  geom_boxplot(color = 'black', fill = '#099DD9') + 
  coord_cartesian(ylim = (xlim = c(0, 25)));
...
ggplot(aes(x = quality.cat, y = citric), data = ww) +
  geom_boxplot(color = 'black', fill = '#099DD9') + 
...
# Extra 101 adds count n to the output.
...
```{r plot-two, fig.cap="Recursive partitioning tree using 'rpart' package"}
